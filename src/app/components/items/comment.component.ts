...
import { UserRole } from '../../api/types/users/user-role';
          ...
          <div class="flex flex-row align-center gap-x-4">
            ...
            <div class="flex flex-row grow justify-end flex-wrap"
              ...
              <app-outlined-button class="text-[14px]" [text]="comment.rating.yayRatings.toString()" [icon]="faThumbsUp"
                [enabled]="ratingButtonsEnabled" [emphasize]="comment.rating.ownRating === likeValue" (click)="like()"></app-outlined-button>
              <app-outlined-button class="text-[14px]" [text]="comment.rating.booRatings.toString()" [icon]="faThumbsDown"
                [enabled]="ratingButtonsEnabled" [emphasize]="comment.rating.ownRating === dislikeValue" (click)="dislike()"></app-outlined-button>
            </div>
          </div>
        </div>
      </app-user-wrapper>
...
export class CommentComponent {
  ...
  likeValue: RatingType = RatingType.Like;
  dislikeValue: RatingType = RatingType.Dislike;
  ...
  ngOnInit() {
    this.auth.user.subscribe(user => {
      if (user) {
        ...
        // In the future also show the delete button if the user is the level publisher, but since MinimalLevel doesn't include publisher data,
        // either the comment's parent component would have to determine the visibility of the button (would mean duplicate code),
        // or MinimalLevel would have to include that publisher data, or we just replace the minimal data with full level/user data
        // to undo this chaos
        if (user.userId === this.comment.publisher.userId || user.role >= UserRole.Moderator
  ...
  private handleDeleteResponse(request: Observable<Response>) {
    request.subscribe({
      ...
      next: _ => {
        this.onDelete.emit(); // get the owning list to remove this comment
      }
    });
  }
  ...
  protected delete() {
    ...
    if (this.comment.profile !== undefined) {
      this.handleDeleteResponse(this.client.deleteProfileComment(this.comment.commentId));
    }
    else {
      this.banner.error("Comment Deletion Failed", "Could not determine comment type. If this is a level comment: they are not supported yet.");
      return;
    }
  }
  ...
  private rate(rating: RatingType): void {
    if (this.comment.profile !== undefined) {
      ...
      this.banner.warn("Can't rate " + this.comment.publisher.username + "'s comment", "The comment's type is unknown. If this is a level comment: they are not supported yet.");
    }
  }
